...
class Model_Finder:
    ...
    def get_best_params_for_naive_bayes(self, train_x, train_y):
        ...
        try:
            ...
            self.logger_object.log(self.file_object, "Naive Bayes best params: " + str(
                    self.grid.best_params_) + " . Exited the get_best_params_for_naive_bayes method fo the Model_Finder class")
            return self.gnb
            ...
            self.logger_object.log(self.file_object,
                                   "Exception occured in get_best_params_for_naive_bayes method of the Model_Finder class. Exception message:: " + str(
                                           e))
            self.logger_object.log(self.file_object,
                                   "Navie Bayes Parameters tuning failed. Exited the get_best_param_for_naive_bayes method of the Model_Finder class")
    ...
    def get_best_params_for_xgboost(self, train_x, train_y):
        ...
        try:
            # initializng with different combination of parameters
            self.param_grid_xgboost = {"n_estimators": [ 50, 100, 130 ], "max_depth": range(3, 11, 1),
            ...
            self.xgb = XGBClassifier(random_state = self.random_state, max_depth = self.max_depth,
                                     n_estimators = self.n_estimators, n_jobs = -1, use_label_encoder = False)

            # training the new model
            self.xgb.fit(train_x, train_y, eval_metric = 'error')
            self.logger_object.log(self.file_object, "XGBoost best params: " + str(
                    self.grid.best_params_) + " . Exited the get_best_params_xgboost method of the Model_Finder class")
            return self.xgb
        except Exception as e:
            self.logger_object.log(self.file_object,
    ...
    def get_best_model(self, train_x, train_y, test_x, test_y):
        """
                                                ...
                                                """
        self.logger_object.log(self.file_object, "Entered the get_best_model method of the Model_Finder classs")
