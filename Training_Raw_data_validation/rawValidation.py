...
class Raw_Data_validation:
    ...
    def moveBadFilesToArchievedBad(self):
        """
            ...
            for filename in onlyfiles:
                if re.match(regex, filename):
                    ...
                    if len(splitAtDot[1]) == LengthOfDateStampInFile:
                        ...
                        self.logger.log(f, "valid File name!! File moced to GoodRaw Folder :: %s" % filename)
                    else:
                        shutil.copy("Training_Batch_Files/" + filename, "Training_Raw_files_validated/Bad_Raw")
                        self.logger.log(f, "Invaid File name!! File moved to Bad Raw Folder :: %s" % filename)
                else:
                    shutil.copy("Training_Batch_Files/" + filename, "Training_Raw_files_validated/Bad_Raw")
    ...
    def validateMissingValuesInWholeColumns(self):
        """
            ...
            for file in listdir("Training_Raw_files_validated/Good_Raw/"):
                ...
                for columns in csv:
                    if (len(csv[columns]) - csv[columns].count()) == len(csv[columns]):
                        ...
                        self.logger.log(f, "Invalid Columns for the file!! File moved to Bad Raw Folder :: %s" % file)
                        break
                if count == 0:
                    csv.rename(columns={"Unnamed: 0": "Card"}, inplace=True)
